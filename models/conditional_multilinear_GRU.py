...
class network(nn.Module):
    ...
    def __init__(self, inp_size, hidden_size_1,hidden_size_2,hidden_size_3,output_size):
        ...
        self.linear_1 = nn.Linear(hidden_size_2+self.condition_len,hidden_size_3)
        self.linear_2 = nn.Linear(hidden_size_3+hidden_size_2+hidden_size_1+ self.condition_len,output_size)
    ...
    def forward_unlooped(self,data,characters, train=True,cuda=True):
        ...
        ############################ 3th Linear
        inp_3 = torch.cat([hidden_2,final_conditional],1)
        hidden_3 = self.linear_1(inp_3)
    ...
    def forward_looped(self,characters,pred_limit,cuda=True,no_sigma= False):
        ...
        while (jter<pred_limit):
            ...
            inp_3 = torch.cat([output_2[0],conditioning_inp[0]],1)
            output_3 = self.linear_1(inp_3)
            
            inp_4 = torch.cat([output_3,output_2[0],output_1[0],conditioning_inp[0]],1)
            output = self.linear_2(inp_4)
